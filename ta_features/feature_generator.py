...
ZSCORE
"""
def slide_features(feature, shift_length=5, shift_step=1):
    cols = feature.columns
    for col in cols:
        for day in range(1, shift_length):
            if col not in ['symbol', 'date']:
                cname = '{0}_{1:02d}'.format(col, day)
                feature[cname] = feature[col].shift(day * shift_step)
    return feature
